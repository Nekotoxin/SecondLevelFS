...
void FileManager::Creat()
{
	...
	if ( NULL == pInode )
	{
		 ...
		 * 所表示的权限内容是一样的。
		 */
		this->Open1(pInode, File::FILE_FWRITE, 2);
	}
	else
	{

		this->Open1(pInode, File::FILE_FWRITE, 1);
...
void FileManager::Open1(Inode* pInode, int mode, int trf)
{
	...
	if (trf != 2)
	{
		if ( mode & File::FILE_FREAD )
		{
			/* 检查读权限 */
			this->Access(pInode, Inode::IREAD);
		}
		if ( mode & File::FILE_FWRITE )
		{
			/* 检查写权限 */
	...
	pFile->f_flag = mode & (File::FILE_FREAD | File::FILE_FWRITE);
	pFile->f_inode = pInode;
	/* 特殊设备打开函数 */
	//pInode->OpenI(mode & File::FILE_FWRITE);
...
void FileManager::Seek()
{
	...
	/* 管道文件不允许seek */
	if ( pFile->f_flag & File::FILE_FPIPE )
	{
		u.u_error =  ESPIPE;
		return;
	}
...
{
	/* 直接调用Rdwr()函数即可 */
	this->Rdwr(File::FILE_FREAD);
}

void FileManager::Write()
{
	/* 直接调用Rdwr()函数即可 */
	this->Rdwr(File::FILE_FWRITE);
}
