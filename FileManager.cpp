...
INode *FileManager::NameI(enum DirectorySearchMode mode) {
    ...
    while (1) {
        ...
        memcpy(g_UserCall.dbuf, g_UserCall.dirp.data() + index,
               (nindex == (unsigned int) string::npos ? g_UserCall.dirp.length() : nindex) - index);
        index = nindex + 1;
        ...
        while (1) {
            ...
            memcpy(&g_UserCall.dent, pCache->addr + (g_UserCall.IOParam.offset % INode::BLOCK_SIZE),
                   sizeof(g_UserCall.dent));
